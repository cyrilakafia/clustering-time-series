...
#         rect = patches.Rectangle((start, start), c, c, linewidth=5, edgecolor='lime', facecolor='none')
#         start += c
#         ax.add_patch(rect)
#     ax.set_xlim([-.25, N+.25])
#     ax.set_ylim([-.25, N+.25])
...
def vis_heatmap(cluster_ids, show_dendrogram=False):
    if len(cluster_ids) == 0:
        print("Error: Empty cluster_ids list.")
    ...
    best_gibbs = np.argmin(sim_mat_dists)
    new_idx = np.argsort(cluster_ids[best_gibbs])
    heatmap(sim_mat.iloc[new_idx, new_idx].T, ax=ax, cmap='coolwarm')
