...
for _ in range(5):
    ...
    obs = np.hstack([np.random.binomial(225, p1, size=(100,)), np.random.binomial(225, p2, size=(50,)), np.random.binomial(225, p1, size=(250,))])
    obs_all.append(obs)
    
# Inhibted, unsustained neurons 
for _ in range(5):
    l1 = np.random.uniform(10, 15)
    l2 = l1 * np.exp(-1)
    p1 = l1 * delta
...
def calc_bssm_log_like_sim(ob, param, n):
    jump, log_var = param
    p_init = ob[:100].sum() / (45 * 5 * 100) 
    mean_init = np.log(p_init / (1 - p_init))
    var = np.exp(log_var)
    return bssm_log_likelihood(ob[100:], var=var, n_particles=64, n_trials=45*5, 
...
output = infer_dp(obs_all, calc_bssm_log_like_sim, 1.0, samp_base, base_logpdf, n_gibbs_iters=10000, 
                  sds_mh_proposal=1/2 * np.array([0.5, 0.5]), n_aux=5, dump_file='pickle/sim.p', seed=SEED)
